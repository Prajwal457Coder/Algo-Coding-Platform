...
const AdminVideo = () => {
  ...
  // The original crashed here if `error` was a plain string (from fetchProblems)
  // rather than an axios error object (from handleDelete) — this handles both.
    ...
    <div className="algo-video-page">
      ...
      <div className="algo-hero">
        ...
        <div className="algo-hero__inner">
          <p className="algo-hero__eyebrow">Video management</p>
          <h1 className="algo-hero__title">
            manage<span>_videos</span>
          </h1>
          <p className="algo-hero__subtitle">
            {problems.length} problem{problems.length === 1 ? '' : 's'} — upload or remove editorial videos.
          </p>
        </div>
      </div>
      ...
      <div className="algo-content">
        ...
        {loading ? (
          <div className="algo-skeleton-list" style={{ marginTop: error ? '1rem' : 0 }}>
          ...
          <div className="algo-list" style={{ marginTop: error ? '1rem' : 0 }}>
            {problems.map((problem, index) => (
              ...
              >
                ...
                <div className="algo-row__actions">
                  <NavLink to={`/admin/upload/${problem._id}`} className="algo-btn algo-btn--upload">
                    <UploadCloud size={14} />
                    Upload
                  </NavLink>
                  <button
                    onClick={() => handleDelete(problem._id)}
                    className="algo-btn algo-btn--delete"
                    disabled={deletingId === problem._id}
                  >
                    {deletingId === problem._id ? (
                      <span className="algo-spin"><Trash2 size={14} /></span>
                    ) : (
                      <Trash2 size={14} />
                    )}
                    Delete
