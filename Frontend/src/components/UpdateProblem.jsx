...
// Orders startCode/referenceSolution arrays to always be [C++, Java, JavaScript],
// regardless of the order the backend returns them in.
...
function UpdateProblem() {
  ...
  // Load the existing problem and pre-fill the form
  useEffect(() => {
    const fetchProblem = async () => {
      setIsFetching(true);
      setFetchError('');
      try {
        // NOTE: adjust this endpoint if your backend uses a different route
        ...
        reset({
          ...
          startCode: sortByLanguage(data.startCode),
          referenceSolution: sortByLanguage(data.referenceSolution)
        });
      } catch (error) {
        setFetchError(error.response?.data?.message || error.message || 'Failed to load problem');
      } finally {
        setIsFetching(false);
      }
    };
  ...
  if (isFetching) {
    return (
      <div className="min-h-screen grid place-items-center bg-base-200/40">
      ...
      <div className="sticky top-0 z-20 border-b border-base-200 bg-base-100/80 backdrop-blur">
        <div className="max-w-4xl mx-auto px-6 py-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold leading-tight">Update Problem</h1>
              <p className="text-sm text-base-content/60">
                Edit the fields below and save to update this problem.
              </p>
            </div>
          </div>
      ...
      >
        ...
        <div className="sticky bottom-0 z-10 -mx-6 px-6 py-4 bg-gradient-to-t from-base-200/40 via-base-200/40 to-transparent backdrop-blur-sm">
          ...
          >
            ...
            {isSubmitting ? 'Saving Changes...' : 'Update Problem'}
          </button>
        </div>
      </form>
    </div>
